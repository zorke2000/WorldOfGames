...
def add_score(difficulty_level):
    ...
    Util.my_log("Current score: %s" % score, msg_type="info")
    # calc new score after latest win
    new_points = difficulty_level*3
    Util.my_log("For this game, the player scored %s new points." % new_points, msg_type="info")
    score += new_points
    Util.my_log("New score: %s" % score, msg_type="info")
    write_to_score_file(score)


def write_to_score_file(score):
    with open(Util.SCORES_FILE, "w") as file:
        file.write(str(score))


def read_score_file():
    if path.exists(Util.SCORES_FILE):
        with open(Util.SCORES_FILE, "r") as file:
            return int(file.readline())
    else:
        Util.my_log("Can't read from Score file! (code %s)" % Util.BAD_RETURN_CODE, msg_type="err")
        write_to_score_file(0)
        Util.my_log("Created the Score file. Reset score to 0...", msg_type="info")
        return Util.BAD_RETURN_CODE


def reset_score_file():
    if not path.exists(Util.SCORES_FILE):
        Util.my_log("Score file not exists yet... Create & init it with score 0.", msg_type="info")
    else:
        Util.my_log("Score file already exists. Reset it to score 0.", msg_type="info")
    write_to_score_file(0)
